    ...
    def run(self):
        ...
        for event in gamepad.read_loop():
             ...
             if event.type == ecodes.EV_ABS:
                #Joystick
                if 'ABS_X' in codename:
                    
                    if 40000 <= event.value <= 59000 and direction != 0.5:
                        print("joystick soft droite")
                        kit.servo[0].angle = SOFT_RIGHT
                        direction = 0.5
                    elif 59000 < event.value and direction != 1:
                        print("joystick droite")
                        direction = 1
                        kit.servo[0].angle = DIR_RIGHT
                        
                    elif 10000 <= event.value <= 30000 and direction != -0.5:
                        print("joystick soft gauche")
                        kit.servo[0].angle = SOFT_LEFT
                        direction = -0.5
                    elif event.value < 10000 and direction != -1:
                        print("joystick gauche")
                        kit.servo[0].angle = DIR_LEFT
                        direction = -1
                        
                    elif 30000 < event.value < 40000 and direction != 0:
                        print("joystick repos")
                        kit.servo[0].angle = DIR_STRAIGHT
                        direction = 0                       
